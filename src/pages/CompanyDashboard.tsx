...
/* CandidateReport shape we expect from backend */
...
export default function CompanyDashboard() {
  ...
  const [reportedReports, setReportedReports] = useState<CandidateReport[]>([]);
  ...
  /**
   * Fetch reports from backend and update both reportedReports state
   * and mark matching HR contacts with reported = true.
   *
   * Backend endpoint assumed: GET /api/company/reports
   * Response: array of CandidateReport objects (see interface above)
   */
  const loadReportedReports = async () => {
    try {
      setLoadingMessage("Loading reported HR data...");
      const response = await API.get("/api/company/reports");
      const reports: CandidateReport[] = Array.isArray(response.data) ? response.data : [];

      setReportedReports(reports);

      // mark hrContacts that match any report's email or phone
      setHrContacts(prev => {
        if (!reports.length) return prev.map(c => ({ ...c, reported: false }));
        return prev.map(c => {
          const matched = reports.some(r =>
            (r.hrEmail && c.email && r.hrEmail.toLowerCase() === c.email.toLowerCase()) ||
            (r.hrPhone && c.phone && r.hrPhone === c.phone)
          );
          return { ...c, reported: Boolean(matched) };
        });
      });
    } catch (error: any) {
      console.error("Failed to load reported HRs:", error);
      setReportedReports([]);
      alert("Failed to load reported HRs: " + (error.response?.data?.message || error.message));
    } finally {
      setLoadingMessage("");
    }
  };
  ...
  try {
    await API.delete(`/api/company/reports/${reportId}`);

    // Remove the report from UI immediately  
    setReportedReports(prev => prev.filter(r => r.id !== reportId));

    // Refresh HR matching after deletion
    setHrContacts(prev => {
      return prev.map(c => {
        const stillReported = prev.some(r =>
          (r.email && c.email && r.email.toLowerCase() === c.email.toLowerCase()) ||
          (r.phone && c.phone && r.phone === c.phone)
        );
        return { ...c, reported: stillReported };
      });
    });
  ...
  const filteredReports = reportedReports.filter(r =>
    (r.hrName?.toLowerCase().includes(searchQuery.toLowerCase()) ?? false) ||
    (r.hrEmail?.toLowerCase().includes(searchQuery.toLowerCase()) ?? false) ||
    (r.hrPhone?.includes(searchQuery) ?? false) ||
    (r.reason?.toLowerCase().includes(searchQuery.toLowerCase()) ?? false)
  );
            ...
            {filteredReports.length === 0 ? (
              ...
              <div className="space-y-4">
                {filteredReports.map(report => (
                  <div
                    key={report.id}
                    className="p-4 rounded-lg border border-red-300 bg-red-50 flex justify-between items-start transition-all hover:shadow-md"
                  >
                    <div className="flex-1">
                      <div className="font-semibold text-slate-900 flex items-center">
                        {report.hrName || report.hrEmail}
                        <span className="ml-2 text-xs text-red-600 px-2 py-0.5 rounded bg-red-100 font-medium">
                          Reported
                        </span>
                      </div>
                      ...
                      <div className="text-xs text-slate-400 mt-2">
                        Status: <span className="font-medium">{report.status ?? "PENDING"}</span>
                        {report.reportedAt && (
